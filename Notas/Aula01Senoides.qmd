...
Em regime permanente senoidal, a tensão ou corrente elétrica é descrita por uma **senoide**:

$$v(t) = V_m \cos(\omega t + \phi)$$

---

## O Sinal Senoidal — Parâmetros
...
Pela relação de Euler: $e^{j\theta} = \cos\theta + j\sin\theta$
...
## Formas de Representação Fasorial — Conversões
...
**Domínio do tempo → Domínio fasorial: $v(t) = V_m\cos(\omega t + \phi) \quad\longleftrightarrow\quad \mathbf{V} = V_m\angle\phi$

---

## Transformação Tempo → Fasor — Exemplos
...
## Resumo: Domínio do Tempo vs. Fasorial (cont.)
...
3. Mostre, a partir da definição de fasor, que a integral de $v(t) = V_m\cos(\omega t+\phi)$ corresponde, no domínio fasorial, a $\mathbf{V}/(j\omega)$.
