---
title: "Impedância, Admitância e Associações em CA"
...
%| fig-align: center
%| fig-width: 8
\input{imgs/tikz/DivisorTensaoCA.tikz}
```
...
%| fig-align: center
%| fig-width: 8
\input{imgs/tikz/DivisorCorrenteCA.tikz}
```
...
## Transformação Estrela-Triângulo (Y-$\Delta$)
...
%| fig-align: center
%| fig-width: 12
\input{imgs/tikz/TransformacaoYDelta.tikz}
```
...
## Transformação Y-$\Delta$ — Exemplo Numérico
...
6. No divisor de tensão do primeiro exemplo desta seção, verifique por soma fasorial que $\mathbf{V}_1+\mathbf{V}_2 = \mathbf{V}_s$.
7. Um triângulo tem $\mathbf{Z}_{12}=10\ \Omega$, $\mathbf{Z}_{23}=j5\ \Omega$ e $\mathbf{Z}_{31}=-j5\ \Omega$. Calcule a estrela equivalente $\mathbf{Z}_1,\mathbf{Z}_2,\mathbf{Z}_3$.
